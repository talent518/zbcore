...
class ZBCore {
	...
	function message($message, $backurl = '', $status = false, $timeout = 3) {
		...
		if(IN_AJAX) {
			...
			$this->echoPage(array(
				'Content-type' => 'text/xml; charset=' . CFG()->charset
			), $content);
		} else {
			$this->setCall();
			is_array($message) && extract($message);
			$this->setVar('head', array(
				'title' => $status ? '成功提醒' : '失败提醒'
			));
	...
	function echoJson($json) {
		$this->obclean();
		$this->echoPage(array(
			'Content-type' => 'application/json; charset=' . CFG()->charset
		), function_exists('json_encode') ? json_encode($json) : L('json')->encode($json));
	}
	...
	function obEchoPage() {
		...
		if(IN_AJAX) {
			...
			$this->echoPage(array(
				'Content-type' => IN_MIME . '; charset=' . CFG()->charset
			), $content);
		}
	}
