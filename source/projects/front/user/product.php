...
class CtrlUserProduct extends CtrlBase{
	var $id=0;
	...
	function onList(){
		if($this->is_submit('list')){
			...
			$this->display('user/product/list');
		}
	}
	function onAdd(){
	...
	function onEdit(){
		if($this->is_submit('edit')){
			...
			$this->display('user/product/edit');
		}
	}
	function onDrop(){
		if($this->is_submit('drop')){
			...
			$this->display('user/product/drop');
		}else
			$this->message('产品不存在！');
	}
	function onUpload(){
