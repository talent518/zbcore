...
实例:
	$('#float').float('center');
	$('#float').float('left center');
	$('#float').float('center top','#div');
	$('#float').float('right bottom','#div');
	$('#float').float({top:20,X:'center'});
	$('#float').float({bottom:100,X:'center'},'#div');
	$('#float').float({Y:'center',X:'right'});
	$('#float').float({'center','#div');
*/
...
$.fn.float=function(position,parent){
	...
	return this.each(function(){
		var loc={},isChild=$(parent).children().index(this)>-1;
		if(pos.X){
			switch(pos.X){
				case('left'):
					loc.left=0;
		...
		var css_pos=(isIE6 || parent!=window)?'absolute':'fixed';
