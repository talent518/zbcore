...
$.Weather.get=function(province,city,conainter){
	...
	$.getScript('http://weather.news.qq.com/inc/minisite_'+_city_+'.js',function(){
		try{
			$(conainter).html('<a href="http://weather.news.qq.com/preend.htm?dc'+_city_+'.htm" target="_blank">'+__minisite__weather__+'</a>');
		}catch(e){}
	});
};
...
$.fn.weather=function(){
	...
	var ok=function(){
		var province=null;
		var city=null;
		var ipAddress=$.cookie('QQ_IPAddress');
		...
		$.Weather.get(province,city,$this);
	};

	if (!$.cookie('QQ_IPAddress')){
		$.getScript('http://fw.qq.com:80/ipaddress',function(){
			if (typeof IPData!='undefined'){
				$.cookie('QQ_IPAddress',IPData[2]+','+IPData[3]);
				ok();
			}
		});
	}else{
