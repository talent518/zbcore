{if !IN_AJAX}
	...
	<div id="hd" class="wp">
		...
		<ol>
		{var $navs=M('category')->get_list_by_where('pid=0');$w=100/count($navs);}
		{loop $navs $i $r}
			<li style="width:{$w}%;" class="{if GET('method')=='category' && in_array($catid,M('category')->get_child($i))} active{/if}"><a href="{link method=category id=$i}" title="{$r.cat_name}">{$r.cat_name}</a></li>
		{/loop}
		</ol>
	</div>
