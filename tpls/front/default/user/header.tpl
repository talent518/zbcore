	...
	<div id="hd" class="wp">
		<p>
			...
			{if $LOGINED}
				...
				<a href="{link ctrl=user}">用户中心</a><span class="split">|</span>
				{if $MEMBER[ismanage]}
					<a href="{link proj=admin}" target="_blank">管理中心</a><span class="split">|</span>
				{/if}
				<a href="{link ctrl=user method=logout}">退出</a>
			{else}
				<a href="{link ctrl=user method=login}">登录</a> | <a href="{link ctrl=user method=register}">注册</a>
			{/if}
			</php>
