<h1 class="head">
	<a href="{link ctrl=user.product method=add id=$id}" class="add button" title="添加产品">添加产品</a>
	产品管理
</h1>
<form id="listform" class="formtable" action="{link ctrl=user.product method=list id=$id}" method="post">
		...
		{if $list}
			...
			<tr class="{if $i%2==1}odd{else}even{/if}">{$i++;}
				...
				<td>
					...
					<a href="{link ctrl=user.product method=edit id=$r.prod_id}" class="edit" title="编辑产品">编辑</a>
					<span class="split">|</span>
					<a href="{link ctrl=user.product method=drop id=$r.prod_id}" class="drop" title="删除产品">删除</a>
				</td>
			</tr>
			{/loop}
			{if PAGES>1}
